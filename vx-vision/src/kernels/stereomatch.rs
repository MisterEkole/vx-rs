...
impl StereoMatcher {
    ...
    pub fn new(ctx: &Context) -> Result<Self> {
        ...
        let hamming_func = ctx
            .library()
            .newFunctionWithName(hamming_name)
            .ok_or(Error::ShaderMissing("stereo_hamming".into()))?;
        let extract_func = ctx
            .library()
    ...
    pub fn run(
        ...
        assert_eq!(
            left_kpts.len(),
            left_descs.len(),
            "left_kpts and left_descs length mismatch"
        );
        assert_eq!(
            right_kpts.len(),
            right_descs.len(),
            "right_kpts and right_descs length mismatch"
        );
        ...
        let dist_matrix: UnifiedBuffer<u16> = UnifiedBuffer::new(ctx.device(), n_left * n_right)?;
