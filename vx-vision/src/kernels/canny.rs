...
use crate::kernels::sobel::{SobelEncodedState, SobelFilter};
...
impl CannyDetector {
    pub fn new(ctx: &Context) -> Result<Self> {
        ...
        let nms_func = ctx
            .library()
            .newFunctionWithName(nms_name)
            .ok_or_else(|| Error::ShaderMissing("canny_nms".into()))?;
        let hyst_func = ctx
            .library()
            .newFunctionWithName(hyst_name)
    ...
    pub fn detect(&self, ctx: &Context, input: &Texture, config: &CannyConfig) -> Result<Texture> {
        let w = input.width();
        let h = input.height();
    ...
    pub fn encode(
        ...
        let blur_state = self.blur.encode(
            ctx,
            cmd_buf,
            input,
            &blurred,
