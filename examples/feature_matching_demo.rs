...
fn main() {
    ...
    let desc1 = orb.compute(&ctx, &tex1, &kp1, &pattern, &OrbConfig::default()).expect("ORB1");

    let corners2 = fast.detect(&ctx, &tex2, &fast_cfg).expect("FAST2").corners;
    let scored2 = harris.compute(&ctx, &tex2, &corners2, &harris_cfg).expect("Harris2");
    let nms2 = nms.run(&ctx, &scored2, &nms_cfg).expect("NMS2");
    let kp2: Vec<_> = nms2.into_iter().take(512).collect();
    let desc2 = orb.compute(&ctx, &tex2, &kp2, &pattern, &OrbConfig::default()).expect("ORB2");
    ...
    if !orb_matches.is_empty() {
        let avg_dist: f32 = orb_matches.iter().map(|m| m.distance as f32).sum::<f32>() / orb_matches.len() as f32;
        let avg_ratio: f32 = orb_matches.iter().map(|m| m.ratio).sum::<f32>() / orb_matches.len() as f32;
    ...
    let feat2 = sift.detect_and_describe(&ctx, &tex2, &sift_cfg).expect("SIFT2");
    ...
    if !sift_matches.is_empty() {
        let avg_dist: f32 = sift_matches.iter().map(|m| m.distance).sum::<f32>() / sift_matches.len() as f32;
    ...
    if w1 >= patch_size && h1 >= patch_size && w2 >= patch_size && h2 >= patch_size {
        ...
        for y in 0..patch_size as usize {
            for x in 0..patch_size as usize {
                patch[y * patch_size as usize + x] = img1.as_raw()[(cy + y) * w1 as usize + (cx + x)];
            }
        }
        let tpl = ctx.texture_gray8(&patch, patch_size, patch_size).expect("template texture");

        let t_tm = Instant::now();
        let tm_result = tm.match_template(&ctx, &tex2, &tpl).expect("Template match");
        let tm_ms = t_tm.elapsed().as_secs_f64() * 1000.0;

        println!("  Patch from:  ({}, {}) in image 1", cx, cy);
        println!("  Best match:  ({}, {}) in image 2", tm_result.best_x, tm_result.best_y);
    ...
    println!("SIFT:  {} matches in {:.2} ms (detect+match)",
        sift_matches.len(), sift_detect_ms + sift_match_ms);
}
